...
// ----------------------------------------------------------------------------
		...
		$q="SELECT calls.TimeOfCall,calls.Internally,calls.Number,calls.Duration,calls.Way,co.Name,internally.Name FROM calls LEFT JOIN co ON calls.CO = co.CO LEFT JOIN internally ON calls.Internally = internally.Internally where (calls.TimeOfCall>='".$from_date."' AND (calls.TimeOfCall<='".$to_date."')".$additionalReq.") ORDER BY ".$sortBy." ".$order.$limitsP;
		if($debug) echo $q.";<br>";
...
//
				...
				if(empty($export)){
				    ...
				    echo"<td><a href=\"".complitLink($local_type="CoIntDetail",$local_int="$row[1]")."\" title=\"".$GUI_LANG['CallsThroughAnExternalLine']." $NamedLine ".$GUI_LANG['FromInternalPhone']." $row[1] $telephone\">$row[1]</a> $telephone</td>";
				    echo"<TD>${FontColor}$row[2]${FontColorEnd}</TD>";
				    echo"<TD>${FontColor}".sumTotal($row[3],0)."${FontColorEnd}</TD>";
				    echo "</TR>\n";
				}else{
				    $expor_excel->MontaConteudo($linha+2, 0, $row[0]);
				    $expor_excel->MontaConteudo($linha+2, 1, $row[1]." ".$telephone);
