...
// ----------------------------------------------------------------------------
		$q="SELECT calls.Number,count(*),sum(calls.Duration),calls.Way from calls where ((calls.TimeOfCall>='".$from_date."') AND (calls.TimeOfCall<='".$to_date."') ".$additionalReq.") GROUP BY calls.Number ORDER BY ".$sortBy." ".$order.$limitsP;
		if($debug) echo $q."<br>";
...
//
				...
				if(empty($export)){
				    ...
				    echo"<td><a href=\"".complitLink($local_type="NumDetail",$local_num="$row[0]")."\" title=\"".$GUI_LANG['CallsOnTheNumber'].": $numberName\">$row[1]</a></td>";
