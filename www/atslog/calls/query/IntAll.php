...
$qA2="  
INSERT INTO tmp_A SELECT calls.internally,SUM(duration)
from calls
...
// ------------------------------------------------------------------------------------------------------
//
$q="SELECT calls.internally,intphones.name,COUNT(*),
tmp_A.duration,tmp_B.duration,tmp_C.duration,tmp_D.duration
from calls
LEFT JOIN intphones ON calls.internally = intphones.intnumber
LEFT JOIN tmp_A ON calls.internally=tmp_A.internally
LEFT JOIN tmp_B ON calls.internally=tmp_B.internally
LEFT JOIN tmp_C ON calls.internally=tmp_C.internally
LEFT JOIN tmp_D ON calls.internally=tmp_D.internally
where (calls.timeofcall>='".$from_date."'
AND (calls.timeofcall<='".$to_date."')
".$additionalReq."
)
group by calls.internally,intphones.name,tmp_A.duration,tmp_B.duration,tmp_C.duration,tmp_D.duration
ORDER BY ".$sortBy." ".$order.$limitsP;

// Выполним основной запрос изпользуя заранее подготовленный LIMIT
